...
const berthZones = require('./berth-zones.json');
...
const PORTS = [
  ...
  {
    name: 'Los Angeles', lat: 33.74, lon: -118.27, max: capacities['Los Angeles'] ?? 70, berthCapacity: berthCap['Los Angeles'] ?? 80, reliability: 'high', utcOffset: -8,
...
function inAnyBox(lat, lon, zones) {
  return Array.isArray(zones) && zones.some(z => inBox(lat, lon, z));
}
...
module.exports = { PORTS, inBox, inAnyBox, normalizeDestination };
